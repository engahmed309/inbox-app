...
export default function ConversationsScreen() {
  ...
  // الشرائح هنا للتصفح بس (اختيار شريحة محفوظة تشوف محادثاتها) — إنشاؤها وتعديلها من الإعدادات بس
  ...
  const fetchStaticLists = useCallback(async () => {
    ...
    // الشرائح أدمن بس — لو الموظف مش أدمن الباك إند بيرجع 403 وبنسيب القايمة فاضية بهدوء. إنشاء/تعديل
    // الشرائح بقى بس من الإعدادات، هنا بس تصفح شريحة محفوظة (تشوف محادثاتها)
    try {
      ...
      <div className={`${mobileMenuOpen ? 'flex' : 'hidden'} lg:flex fixed lg:static inset-y-0 start-0 z-40 lg:z-auto w-72 ${sidebarOpen ? 'lg:w-72' : 'lg:w-16'} flex-col bg-surface-2 border-e border-surface-3 transition-all duration-200 flex-shrink-0`}>
        ...
        <div className={`flex flex-col gap-2 border-b border-surface-3 py-2.5 ${expanded ? 'px-3' : 'items-center'}`}>
          ...
          <div className={`flex items-center gap-1 ${expanded ? 'flex-wrap' : 'flex-col'}`}>
            ...
            {(agent?.role === 'admin' || ['comments', 'both'].includes(agent?.access_scope)) && (
              <button onClick={() => navigate('/comments')} title={t('comments.title')}
                className="w-8 h-8 flex items-center justify-center text-fg-muted hover:text-fg rounded-lg hover:bg-surface-3 transition-colors">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {/* الشرائح (Segments) — تصفح شريحة محفوظة بس (تشوف محادثاتها)، أدمن بس. إنشاء/تعديل/حذف
              الشرائح بقى من الإعدادات بس، مش من هنا */}
          {agent?.role === 'admin' && expanded && segments.length > 0 && (
            <div className="border-t border-surface-3 py-2">
              <p className="px-3 pb-1.5 pt-1 text-[11px] font-semibold text-fg-subtle">{t('conversations.segments.heading')}</p>
              {selectedSegmentId && (
              ...
              {segments.map(seg => (
                <button key={seg.id} onClick={() => { setSelectedSegmentId(prev => prev === seg.id ? null : seg.id); setMobileMenuOpen(false) }}
                  className={`w-full flex items-center gap-2 px-3 py-2 text-sm transition-colors rounded-lg mx-auto max-w-[calc(100%-1rem)] ${selectedSegmentId === seg.id ? 'bg-surface-3 text-fg' : 'text-fg-muted hover:bg-surface-3/60'}`}>
